// Ported from upstream ESPAltherma include/converters.h (MIT, (c) 2020 Raomin).
//
// The conversion table is field-proven against real hardware and is carried
// over literally, case for case. Three deliberate deviations are marked
// DEVIATION below; everything else differs only in plumbing (Serial.printf ->
// ESP_LOGx, and the C facade declared in converters.h).
...
#include "model_config.h"   // pulls in labeldef.h and def/<model>.h -> labelDefs[]

static const char *TAG = "conv";

static const size_t kLabelCount = sizeof(labelDefs) / sizeof(labelDefs[0]);
...
public:
    ...
    void getLabels(char registryID, LabelDef *ret[], int &num)
    {
        ...
        for (size_t i = 0; i < kLabelCount; i++) {
            if (labelDefs[i].registryID == (int)(unsigned char)registryID) {
                ret[num++] = &labelDefs[i];
            }
        }
    }
    ...
    void readRegistryValues(const unsigned char *data, unsigned char protocol)
    {
        if (protocol == 'S') {
    ...
    void readRegistryValues(char registryID, const unsigned char *data, unsigned int offset)
    {
        int num = 0;
        LabelDef *labels[128];
    ...
    {
        // R32 default; R410A/R22 selected via RType (conv 800-803)
    ...
    void convert(LabelDef *def, const unsigned char *data)
    {
        def->asString[0] = 0;
        ...
        {
        case 100:
            strlcat(def->asString, (const char *)data, num);
            return;
        case 101:
        ...
        case 204:
            ...
            return;
        case 211:
            // DEVIATION 1 (behaviour preserved, warning removed): upstream reads
            // "if (data == 0)", comparing the POINTER to null, so the OFF branch
            // is unreachable and every value falls through to the numeric one.
            // Intent was almost certainly data[0] == 0. Kept as-is so readings
            // match upstream byte for byte; not reached by protocol S anyway.
            if (data == NULL) {
        ...
        {
            int hi = data[0] >> 4;
            int lo = (int)(data[0] & 15);
            sprintf(def->asString, "{0:%d}{1:%d}", hi, lo);
            return;
        }
        case 201:
        case 217:
        ...
        // DEVIATION 2 (behaviour preserved): upstream guards this with
        // "if (dblData != NAN)", which is always true because NaN compares
        // unequal to everything, itself included. So the value is always
        // formatted, printing "nan" when a conversion produced one. Kept.
        sprintf(def->asString, "%g", dblData);
        ESP_LOGV(TAG, "%s -> %s", def->label, def->asString);
    }

private:
    void convertTable300(const unsigned char *data, int tableID, char *ret)
    {
        char b = 1;
        b = (char)(b << tableID % 10);
        if ((data[0] & b) > 0) {
    ...
    void convertTable203(const unsigned char *data, char *ret)
    {
        switch (data[0]) {
        case 0: strcat(ret, "Normal");  break;
        case 1: strcat(ret, "Error");   break;
        case 2: strcat(ret, "Warning"); break;
        case 3: strcat(ret, "Caution"); break;
        default: strcat(ret, "-");
        }
    }

    void convertTable204(const unsigned char *data, char *ret)
    {
    ...
    double convertTable312(const unsigned char *data)
    {
    ...
    void convertTable315(const unsigned char *data, char *ret)
    {
        char b = 240 & data[0];
        b = (char)(b >> 4);
        switch (b) {
        case 0: strcat(ret, "Stop");           break;
        case 1: strcat(ret, "Heating");        break;
        case 2: strcat(ret, "Cooling");        break;
        case 3: strcat(ret, "??");             break;
        case 4: strcat(ret, "DHW");            break;
        case 5: strcat(ret, "Heating + DHW");  break;
        case 6: strcat(ret, "Cooling + DHW");  break;
        default: strcat(ret, "-");
        }
    }

    void convertTable316(const unsigned char *data, char *ret)
    {
        char b = 240 & data[0];
        b = (char)(b >> 4);
        switch (b) {
        case 0: strcat(ret, "H/P only");    break;
        case 1: strcat(ret, "Hybrid");      break;
        case 2: strcat(ret, "Boiler only"); break;
        default: strcat(ret, "Unknown");
        }
    }

    void convertTable200(const unsigned char *data, char *ret)
    {
        if (data[0] == 0) {
            strcat(ret, "OFF");
        } else {
    ...
    // 201 / 217
    void convertTable217(const unsigned char *data, char *ret)
    {
        static const char *r217[] = {
            "Fan Only", "Heating", "Cooling", "Auto", "Ventilation",
            "Auto Cool", "Auto Heat", "Dry", "Aux.", "Cooling Storage",
            "Heating Storage", "UseStrdThrm(cl)1", "UseStrdThrm(cl)2",
            "UseStrdThrm(cl)3", "UseStrdThrm(cl)4", "UseStrdThrm(ht)1",
            "UseStrdThrm(ht)2", "UseStrdThrm(ht)3", "UseStrdThrm(ht)4"};
        const int kStates = (int)(sizeof(r217) / sizeof(r217[0]));

        // DEVIATION 3 (bug fix, deliberate): upstream indexes this 19-entry
        // table with an unchecked byte and prints it through a non-literal
        // format string. Any register value above 18 reads off the end of the
        // array. That is directly in this unit's path - PROTOCOL_S.h maps 0x55
        // offset 0 "Operation Mode" to convid 201 - so the index is bounded
        // here and out-of-range states print as a number instead.
        int v = data[0];
        if (v >= 0 && v < kStates) {
            strlcpy(ret, r217[v], 30);
        } else {
            snprintf(ret, 30, "State %d", v);
        }
    }

    unsigned short getUnsignedValue(const unsigned char *data, int dataSize, int cnvflg)
    {
    ...
    short getSignedValue(const unsigned char *data, int datasize, int cnvflg)
    {
...
{
    s_converter.RType = rtype;
}

int converter_refrigerant(void)
{
    return s_converter.RType;
}

size_t converter_registry_ids(uint8_t *out, size_t max)
{
    size_t n = 0;
    for (size_t i = 0; i < kLabelCount && n < max; i++) {
        uint8_t reg = (uint8_t)labelDefs[i].registryID;
        bool seen = false;
        for (size_t j = 0; j < n; j++) {
            if (out[j] == reg) { seen = true; break; }
        }
        if (!seen) {
            out[n++] = reg;
        }
...
{
    s_converter.readRegistryValues(data, (unsigned char)protocol);
}
...
bool converter_label_at(size_t i, uint8_t *reg_id, const char **label, const char **value)
{
    ...
    if (reg_id) *reg_id = (uint8_t)labelDefs[i].registryID;
    if (label)  *label  = labelDefs[i].label;
